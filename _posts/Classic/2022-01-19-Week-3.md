...
```
{'웨스턴 오스트레일리아': '빨강', '노던 준주': '초록', '사우스 오스트레일리아': '초록', '퀸즐랜드': '빨강', '뉴사우스웨일스': '초록', '빅토리아': '빨강', '태즈메이니아': '초록'}
```
...
`generate_grid`는 무작위 알파벳 대문자로 채워진 격자인 2차원 리스트를 생성한다. **random.choice**는 입력받은 Sequence에서 무작위 항목을 반환하는데, 이때 인자로 "ABCDEFGHIJKLMNOPQRSTUVWXYZ" 대신 **string.ascii_uppercase**로 간단하게 나타낼 수 있다.

`display_grid`는 격자를 출력한다.
