...
프림 알고리즘은 최소 신장 트리(이음선의 모음)를 반환한다.
...
class RemovableGraph(Graph[V]):
    ...
    def __repr__(self) -> str:
        ...
        for v in self._vertices:
            desc += f"{str(v)} -> {str(self._edges[self.index_of(v)])}\n"
        return desc
```
...
쾨니히스베르크 다리 문제를 정점 A, B, C, D와 이음선a, b, c, d, e, f, g를 갖는 무향 비가중 그래프로 정형화한다. 이 그래프는 특이하게도 두 정점을 연결하는 이음선이 여러 개 존재한다. 예를 들어, A와 B를 연결하는 이음선은 a, b로 두 개 존재한다. 이러한 그래프를 **다중 그래프**(multigraph)라고 한다.

경로를 도출하는 양상은 미로 문제와 유사하다. 너비 우선 탐색을 사용해 볼 수 있다.
