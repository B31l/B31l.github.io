...
Windows Terminal은 많은 기능과 우수한 편의성을 자랑하는 터미널이지만,

설치하지 않아도 텐서플로우 작업환경을 구성하는 데에는 전혀 문제가 없습니다.

Ubuntu 버전은 **Ubuntu 20.04.4 LTS**를 선택합니다. 

설치가 완료되면 실행하여 사용자 정보(**username** 및 **password**)를 입력합니다.
...
Windows용 Docker 데스크톱은 [이곳](https://docs.docker.com/desktop/windows/install/)에서 설치할 수 있습니다.

설치가 완료되면 몇 가지 환경 설정을 해야 합니다.
...
이 작업은 루트가 아닌 사용자 그룹에 root와 동일한 권한을 부여합니다.

선택적 작업이므로 진행하지 않고 2.4로 건너뛰어도 무방합니다.
...
다음 형식을 통해 리눅스에서 Tensorflow 이미지를 태그할 수 있습니다.
...
2.   태그 변이를 적용하면 각 베이스 태그에는 기능 추가를 할 수 있습니다.
