...
`consistent` 메서드는 **assignment**

`backtracking_search` 메서드는 **assignment**를 깊이 우선 탐색(DFS)과 유사한 방식을 사용해 탐색한 다음 솔루션을 반환한다.
...
| 두 지역 중 하나라도<br>할당이 이루어지지 않은 경우           | 두 지역 모두 할당이 이루어지고<br>충돌이 발생하지 않은 경우  | 두 지역 모두 할당이 이루어지고<br/>충돌이 발생한 경우        |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
...
![](https://github.com/B31l/B31l.github.io/blob/master/img/Classic/australia-2.png?raw=true)
