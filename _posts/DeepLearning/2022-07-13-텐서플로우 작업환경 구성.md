...
이번 단락에서는 윈도우 환경(10 이상)에서 네이티브로 리눅스 실행 파일을 실행하기 위해 WSL 2와 Ubuntu를 설치합니다.
## 1.1 기본 프로그램 설치
...
**Windows Terminal**를 사용하지 않아도 텐서플로우 작업환경을 구성하는 데에는 문제가 없습니다. 하지만 Windows Terminal은 많은 기능을 제공하고 설정이 자유롭기 때문에 설치합니다.
우분투 버전은 **Ubuntu 20.04.4 LTS**를 선택합니다. 설치가 완료되면 실행한 다음, **username** 및 **password**를 입력하는 초기 설정을 진행합니다.
...
버전업을 진행하기 전에, 먼저 가능 여부를 확인해야 합니다.
...
```
위 방법이 작동하지 않으면 Linux 커널 업데이트를 먼저 진행해야 합니다.
...
```
VERSION이 2로 표시되는 것을 확인할 수 있습니다.
...
WSL 2에서 도커를 사용하기 위해 가장 먼저 할 일은 Windows용 Docker 데스크톱을 설치하는 것입니다. Windows용 Docker 데스크톱은 [이곳](https://docs.docker.com/desktop/windows/install/)에서 설치할 수 있으며, 설치가 완료되면 실행한 다음 다음 사진과 같이 설정합니다.
...
Ubuntu를 실행하고 Docker 엔진을 설치하기 위해 몇 가지 준비 운동을 해야 합니다.
...
루트가 아닌 사용자 그룹에 root와 동일한 권한을 부여하면 보다 편하게 Docker 컨테이너를 관리할 수 있습니다. 선택적 작업이므로 진행하지 않고 2.4로 건너뛰어도 무방합니다.
먼저, docker 그룹을 생성합니다.
```shell
...
공식 Tensorflow Docker 이미지는 [tensorflow/tensorflow](https://hub.docker.com/r/tensorflow/tensorflow/) Docker hub 저장소에 있으며, 다음 형식을 통해 리눅스에서 이미지를 태그할 수 있습니다.
...
구성하고자 하는 텐서플로우 작업환경은 최신 버전의 Tensorflow를 사용합니다.
또한 GPU를 지원하며 Jupyter Notebook 역시 포함해야 합니다.
따라서 다음과 같은 형식으로 태그를 만들고 다운로드할 수 있습니다.
``` shell
...
Docker Daemon을 활성화하기 위해선 다음 명령을 입력해 상태를 `start`로 변경합니다. 
```shell
$ sudo service docker start
```
다음 명령으로 지금까지 구성한 작업환경 컨테이너를 실행할 수 있습니다.
실행하고 생성된 링크로 이동하면 주피터 노트북으로 연결됩니다.
```shell
docker run --gpus all -lt -v $(realpath ~/notebooks):/tf/notebooks -p 8888:8888 tensorflow/tensorflow:latest-gpu-jupyter
```
notebooks 폴더로 이동한 뒤, 테스트를 위한 새 ipynb 파일을 생성합니다.
다음 코드는 Tensorflow의 버전을 확인합니다.
```python
...
```
`device_type: "GPU"`이 보인다면 성공입니다.\
> *Note*
만약 주피터 노트북을 종료하려면 우측 상단의 `Quit` 버튼을 클릭합니다.