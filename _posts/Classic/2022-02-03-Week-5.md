...
추운 지방의 생물을 예로 들어보자. 긴 털과 두꺼운 피하지방을 가진 개체는 생존에 적응할 수 있으며, 그렇지 않은 개체에 비해 살아남고 번식할 확률이 크다. 따라서 시간이 지나고 세대가 교체될수록 환경에 적합한 개체는 증가하고, 적합하지 않은 개체는 감소하다 끝내 사라진다.



1. 방정식 $6x - x^2 + 4y - y^2$이 최댓값이 되는 $x$와 $y$를 구하라.

 유전 알고리즘을 필요로 하는 문제는 혹독한 환경과도 같다. x와 y는 
