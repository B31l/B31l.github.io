...
세로운 새대를 시작한다. 각 세대는 염색체로 이루어져 있고, ~
...
재생산을 위해 가장 높은 적합도를 가질 확률이 높은 개체를 선택한다.
...
> *Note*
>
> 파이썬에서는 데코레이터 **@classmethod** 또는 @staticmethod를 사용해 정적 메서드를 구현할 수 있다. 
...
- **pick_roulette**와 **pick_tournamen**
...
위에서 살펴 본 방정식 문제를 유전 알고리즘을 사용해서 해결할 수 있다. 사람이 생각하는것 하고 비슷

Chromosome 클래스를 재정의한다.
